...
goby::acomms::IPGateway::IPGateway(goby::acomms::protobuf::IPGatewayConfig* cfg)
    : Application(cfg), cfg_(*cfg), dccl_goby_nh_("ip_gateway_id_codec_0"),
      dccl_ip_("ip_gateway_id_codec_1"), dccl_udp_("ip_gateway_id_codec_2"),
      dccl_icmp_("ip_gateway_id_codec_3"), tun_fd_(-1),
      total_addresses_((1 << (IPV4_ADDRESS_BITS - cfg_.cidr_netmask_prefix())) -
                       1), // minus one since we don't need to use .255 as broadcast
      local_address_(0), local_modem_id_(0), netmask_(0),
        ...
        if (i != n)
        {
            ...
            switch (cfg_.model_type())
            {
                case protobuf::IPGatewayConfig::UNIFORM:
                    ...
                    break;
                case protobuf::IPGatewayConfig::AUTONOMY_COLLABORATION: freq = ac_freq(i); break;
            }
                                                ...
                                                const std::string& payload)
{
    ...
    boost::bimap<int, int>::right_map::const_iterator dest_it =
        port_map_.right.find(udp_hdr.dest_port());
    ...
    {
        glog.is(WARN) && glog << "No mapping for destination UDP port: " << udp_hdr.dest_port()
                              << ". Unable to send packet." << std::endl;
        return;
    }

    boost::bimap<int, int>::right_map::const_iterator src_it =
        port_map_.right.find(udp_hdr.source_port());
    ...
    else
    {
        ...
        {
            boost::bimap<int, int>::left_map::iterator dyn_port_it =
                port_map_.left.find(dynamic_port_index_);
        ...
        else if (ret > 0)
        {
            ...
            else
            {
                ...
                if (version == 4)
                {
                    ...
                    {
                        default:
                            glog.is(DEBUG1) && glog << "IPv4 Protocol " << ip_hdr.protocol()
                        ...
                        case IPPROTO_UDP:
                        {
                            ...
                            handle_udp_packet(ip_hdr, udp_hdr,
                                              std::string(&buffer[ip_header_size + UDP_HEADER_SIZE],
                                                          ip_hdr.total_length() - ip_header_size -
                                                              UDP_HEADER_SIZE));
                            break;
                        }
                        case IPPROTO_ICMP:
                        {
                            ...
                            glog.is(DEBUG1) && glog << "Received ICMP Packet with header: "
                                                    << icmp_hdr.ShortDebugString() << std::endl;
                            glog.is(DEBUG1) && glog << "ICMP sending is not supported."
                                                    << std::endl;
    ...
    const goby::acomms::protobuf::ModemTransmission& modem_msg)
{
    ...
    for (int i = 0, n = modem_msg.frame_size(); i < n; ++i)
    {
        ...
        {
            boost::bimap<int, int>::left_map::iterator src_it =
                port_map_.left.find(net_header.udp().srcdest_port(0));
            ...
            boost::bimap<int, int>::left_map::const_iterator dest_it =
                port_map_.left.find(net_header.udp().srcdest_port(1));
            int dest_port = dest_it->second;
            if (dest_port < 0)
            {
                glog.is(WARN) &&
                    glog << "No mapping for destination port: " << net_header.udp().srcdest_port(1)
                                               ...
                                               const std::string& payload)
{
    ...
    enum
    {
        IPV4_SOURCE_ADDR_OFFSET = 12,
        IPV4_DEST_ADDR_OFFSET = 16,
        IPV4_CS_OFFSET = 10
    };
    enum
    {
    ...
    std::string udp_pseudo_header = ip_hdr_data.substr(IPV4_SOURCE_ADDR_OFFSET, NET_LONG_BYTES) +
                                    ip_hdr_data.substr(IPV4_DEST_ADDR_OFFSET, NET_LONG_BYTES) +
                                    char(0) + char(IPPROTO_UDP) +
    ...
    const protobuf::IPGatewayICMPControl& control_msg)
{
    ...
    glog.is(DEBUG1) && glog << "Writing ICMP Control message: " << control_msg.DebugString()
                            << std::endl;
...
int goby::acomms::IPGateway::from_src_dest_pair(std::pair<int, int> src_dest)
{
    ...
    if (src == dest || src == goby::acomms::BROADCAST_ID)
        return -1;

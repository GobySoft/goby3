      ...
      moos_resubscribe_required_(false)
{
    ...
    moos_client_.Run(cfg_.moos_server_host().c_str(), cfg_.moos_server_port(),
                     cfg_.base().app_name().c_str(), cfg_.moos_comm_tick());

    glog.is(VERBOSE) && glog << "Waiting to connect to MOOSDB ... " << std::endl;
    ...
    while (!moos_client_.IsConnected())
    {
        ...
        {
            glog.is(DIE) && glog << "Failed to connect to MOOSDB in " << MAX_CONNECTION_TIMEOUT
                                 << " seconds. Check `moos_server_host` and `moos_server_port`"
                                 << std::endl;
        }
    ...
    {
        if (cfg_.pb_pair(i).direction() ==
            protobuf::MOOSGatewayConfig::ProtobufMOOSBridgePair::PB_TO_MOOS)
        {
        ...
        else if (cfg_.pb_pair(i).direction() ==
                 protobuf::MOOSGatewayConfig::ProtobufMOOSBridgePair::MOOS_TO_PB)
        {
...
void goby::moos::MOOSGateway::moos_inbox(CMOOSMsg& msg)
{
    ...
    msg.SetSourceAux(msg.GetSourceAux() + (msg.GetSourceAux().size() ? "/" : "") +
                     application_name());

    glog.is(VERBOSE) && glog << group("to_moos") << msg << std::endl;
...
void goby::moos::MOOSGateway::loop()
{
    ...
    for (CMOOSMsg& msg : moos_msgs)
    {
        ...
        glog.is(VERBOSE) && glog << group("from_moos") << msg << std::endl;
...
void goby::moos::MOOSGateway::check_for_new_moos_variables()
{
    ...
    if (MOOSTime() - dfLastWildCardTime > DEFAULT_WILDCARD_TIME)
    {
        ...
        if (moos_client_.ServerRequest("VAR_SUMMARY", InMail, 2.0, false))
        {
            if (InMail.size() != 1)
            {
                glog.is(WARN) && glog << "ServerRequest for VAR_SUMMARY returned incorrect mail "
                                         "size (should be one)";
                return;
            }
            ...
            for (const std::string& s : all_var)
            {
                if (!subscribed_vars_.count(s))
                {
                    ...
                    {
                        glog.is(VERBOSE) && glog << "moos_client_.Register for " << s << std::endl;
...
bool goby::moos::MOOSGateway::clears_subscribe_filters(const std::string& moos_variable)
{
    ...
    {
        if (moos_variable.compare(0, cfg_.moos_subscribe_filter(i).size(),
                                  cfg_.moos_subscribe_filter(i)) == 0)
            return true;
    }
    return false;
}
...
bool MOOSGateway_OnConnect(void* pParam)
{
    if (pParam)
    {
        ...
        {
            for (std::set<std::string>::const_iterator it = pApp->subscribed_vars_.begin(),
                                                       end = pApp->subscribed_vars_.end();
                 it != end; ++it)
            {

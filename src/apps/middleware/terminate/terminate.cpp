...
namespace goby
{
                ...
                interprocess().subscribe<
                    groups::terminate_response, protobuf::TerminateResponse>(
                        [this](const protobuf::TerminateResponse& response)
                        {
                            ...
                            {
                                glog.is_debug2() && glog << "Received terminate response from our target PID: " << response.target_pid() << std::endl;
                                waiting_for_response_pids_.erase(pid_it);
                                // insert the PID so we can track when it completely quits
                                running_pids_.insert(std::make_pair(response.target_pid(), response.target_name()));
                            }
    ...
    private:
        void loop() override
            {
                ...
                for(auto it = running_pids_.begin(); it != running_pids_.end(); )
                {
                    ...
                    {
                        glog.is_debug2() && glog << "PID: " << it->first << " (was " << it->second << ") has quit." << std::endl;
                ...
                if(waiting_for_response_names_.empty() && waiting_for_response_pids_.empty() && running_pids_.empty())
                {
                ...
                if(now > start_time_ + timeout)
                {
                    if(glog.is_warn())
                    {
                        ...
                        {
                            glog << "Timeout waiting for targets that responded to our request but have not stopped running: ";
        ...
        bool process_exists(unsigned pid)
            { return (0 == kill(pid, 0)); }        

...
class ContentReader
{
  public:
    ...
    using MultipartReader =
        std::function<bool(MultipartContentHeader header, ContentReceiver receiver)>;
...
class ClientImpl
{
  public:
    ...
    Result Put(const std::string& path, ContentProviderWithoutLength content_provider,
               const std::string& content_type);
    Result Put(const std::string& path, const Headers& headers, size_t content_length,
               ContentProvider content_provider, const std::string& content_type);
    ...
    Result Delete(const std::string& path, const char* body, size_t content_length,
                  const std::string& content_type);
    Result Delete(const std::string& path, const Headers& headers, const char* body,
                  size_t content_length, const std::string& content_type);
  ...
  private:
    ...
    send_with_content_provider(Request& req, const char* body, size_t content_length,
                               ContentProvider content_provider,
                               ContentProviderWithoutLength content_provider_without_length,
                               const std::string& content_type, Error& error);
    Result send_with_content_provider(const std::string& method, const std::string& path,
...
inline void duration_to_sec_and_usec(const T& duration, U callback)
{
    ...
    auto usec =
        std::chrono::duration_cast<std::chrono::microseconds>(duration - std::chrono::seconds(sec))
            .count();
    callback(static_cast<time_t>(sec), static_cast<time_t>(usec));
}

inline uint64_t get_header_value_u64(const Headers& headers, const std::string& key, size_t id,
                                     uint64_t def)
{
...
inline Server& Server::set_read_timeout(const std::chrono::duration<Rep, Period>& duration)
{
    detail::duration_to_sec_and_usec(duration,
                                     [&](time_t sec, time_t usec) { set_read_timeout(sec, usec); });
    return *this;
}
...
const char* get_header_value(const Headers& headers, const std::string& key, size_t id = 0,
...
{
    while (b + left < e && is_space_or_tab(b[left])) { left++; }
    while (right > 0 && is_space_or_tab(b[right - 1])) { right--; }
...
#if defined(_WIN32)
    hFile_ = ::CreateFileA(path, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
...
socket_t create_socket(const std::string& host, const std::string& ip, int port, int address_family,
                       int socket_flags, bool tcp_nodelay, SocketOptions socket_options,
                       BindOrConnect bind_or_connect)
{
     ...
     */
        ...
        if (tcp_nodelay)
        {
            auto yes = 1;
#ifdef _WIN32
            setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&yes),
                       sizeof(yes));
#else
            setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const void*>(&yes),
                       sizeof(yes));
#endif
        }
        ...
        if (rp->ai_family == AF_INET6)
        {
            auto no = 0;
#ifdef _WIN32
            setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<const char*>(&no),
                       sizeof(no));
#else
            setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<const void*>(&no),
                       sizeof(no));
#endif
        }
...
inline socket_t create_client_socket(const std::string& host, const std::string& ip, int port,
...
#ifdef _WIN32
                auto timeout =
                    static_cast<uint32_t>(read_timeout_sec * 1000 + read_timeout_usec / 1000);
                setsockopt(sock2, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char*>(&timeout),
                           sizeof(timeout));
#else
                timeval tv;
                tv.tv_sec = static_cast<long>(read_timeout_sec);
                tv.tv_usec = static_cast<decltype(tv.tv_usec)>(read_timeout_usec);
                setsockopt(sock2, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const void*>(&tv),
                           sizeof(tv));
#endif
            }
            {
...
#ifdef _WIN32
                auto timeout =
                    static_cast<uint32_t>(write_timeout_sec * 1000 + write_timeout_usec / 1000);
                setsockopt(sock2, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const char*>(&timeout),
                           sizeof(timeout));
#else
                timeval tv;
                tv.tv_sec = static_cast<long>(write_timeout_sec);
                tv.tv_usec = static_cast<decltype(tv.tv_usec)>(write_timeout_usec);
                setsockopt(sock2, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const void*>(&tv),
                           sizeof(tv));
#endif
            }
...
inline bool can_compress_content_type(const std::string& content_type)
{
    ...
    switch (tag)
    {
        ...
        default: return !content_type.rfind("text/", 0) && tag != "text/event-stream"_t;
    }
}
...
inline gzip_compressor::gzip_compressor()
{
    ...
    is_valid_ =
        deflateInit2(&strm_, Z_DEFAULT_COMPRESSION, Z_DEFLATED, 31, 8, Z_DEFAULT_STRATEGY) == Z_OK;
}
...
inline bool brotli_decompressor::decompress(const char* data, size_t data_length, Callback callback)
{
    ...
    while (decoder_r == BROTLI_DECODER_RESULT_NEEDS_MORE_OUTPUT)
    {
        ...
        decoder_r =
            BrotliDecoderDecompressStream(decoder_s, &avail_in, &next_in, &avail_out,
...
inline const char* get_header_value(const Headers& headers, const std::string& key, size_t id,
                                    const char* def)
{
    auto rng = headers.equal_range(key);
    auto it = rng.first;
...
template <typename T> inline bool parse_header(const char* beg, const char* end, T fn)
{
    ...
    while (beg < end && is_space_or_tab(end[-1])) { end--; }
    ...
    while (p < end && is_space_or_tab(*p)) { p++; }

    if (p < end)
    {
        auto key = std::string(beg, key_end);
        auto val = compare_case_ignore(key, "Location") ? std::string(p, end)
...
inline bool read_content_with_length(Stream& strm, uint64_t len, Progress progress,
...
template <typename T>
inline bool write_content(Stream& strm, const ContentProvider& content_provider, size_t offset,
                          size_t length, T is_shutting_down, Error& error)
{
...
template <typename T>
inline bool write_content(Stream& strm, const ContentProvider& content_provider, size_t offset,
                          size_t length, const T& is_shutting_down)
{
...
class MultipartFormDataParser
{
  ...
  private:
    ...
    void buf_append(const char* data, size_t n)
    {
        ...
        {
            for (size_t i = 0; i < remaining_size; i++) { buf_[i] = buf_[buf_spos_ + i]; }
        }
        buf_spos_ = 0;
        ...
        for (size_t i = 0; i < n; i++) { buf_[buf_epos_ + i] = data[i]; }
        buf_epos_ += n;
    }
...
inline std::string make_multipart_data_boundary()
{
    ...
    for (auto i = 0; i < 16; i++) { result += data[engine() % (sizeof(data) - 1)]; }
...
inline std::pair<size_t, size_t> get_range_offset_and_length(const Request& req,
                                                             size_t content_length, size_t index)
{
...
inline std::string make_content_range_header_field(const std::pair<ssize_t, ssize_t>& range,
...
bool process_multipart_ranges_data(const Request& req, Response& res, const std::string& boundary,
                                   const std::string& content_type, SToken stoken, CToken ctoken,
                                   Content content)
{
...
inline std::pair<size_t, size_t> get_range_offset_and_length(const Request& req,
                                                             const Response& res, size_t index)
{
    auto r = req.ranges[index];
    ...
    bool is_proxy = false)
{
    ...
    std::string response;
    {
        ...
        {
            response = H(H(A1) + ":" + auth.at("nonce") + ":" + nc + ":" + cnonce + ":" + qop +
                         ":" + H(A2));
        }
    }
    ...
    auto field =
        "Digest username=\"" + username + "\", realm=\"" + auth.at("realm") + "\", nonce=\"" +
...
inline bool parse_www_authenticate(const Response& res, std::map<std::string, std::string>& auth,
                                   bool is_proxy)
{
        ...
        if (pos != std::string::npos)
        {
            ...
            else if (type == "Digest")
            {
                ...
                for (auto i = beg; i != std::sregex_iterator(); ++i)
                {
                    ...
                    auto val = m.length(2) > 0 ? s.substr(static_cast<size_t>(m.position(2)),
...
inline std::pair<std::string, std::string>
make_bearer_token_authentication_header(const std::string& token, bool is_proxy = false)
{
...
inline std::vector<MultipartFormData> Request::get_file_values(const std::string& key) const
{
    ...
    for (auto it = rng.first; it != rng.second; it++) { values.push_back(it->second); }
    return values;
}
...
inline void
Response::set_chunked_content_provider(const std::string& content_type,
                                       ContentProviderWithoutLength provider,
                                       ContentProviderResourceReleaser resource_releaser)
{
    set_header("Content-Type", content_type);
...
inline bool PathParamsMatcher::match(Request& request) const
{
    ...
    for (size_t i = 0; i < static_fragments_.size(); ++i)
    {
        ...
        request.path_params.emplace(param_name,
                                    request.path.substr(starting_pos, sep_pos - starting_pos));
...
{
    while (!is_running() && !done_) { std::this_thread::sleep_for(std::chrono::milliseconds{1}); }
}
...
inline bool Server::write_response(Stream& strm, bool close_connection, const Request& req,
                                   Response& res)
{
                                        ...
                                        Response& res, bool need_apply_ranges)
{
    ...
    if (!res.has_header("Content-Length") && res.body.empty() && !res.content_length_ &&
...

    {
        ...
        while (svr_sock_ != INVALID_SOCKET)
        {
...

            {
                ...
                setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const void*>(&tv),
                           sizeof(tv));
#endif
            }
            {
                ...
                setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const void*>(&tv),
                           sizeof(tv));
#endif
            }
                                 ...
                                 std::string& boundary)
{
    ...
    if (res.body.empty())
    {
        if (res.content_length_ > 0)
        {
            ...
            else if (req.ranges.size() == 1)
            {
                ...
                auto content_range =
                    detail::make_content_range_header_field(req.ranges[0], res.content_length_);
    ...
    else
    {
        ...
        {
            auto content_range =
                detail::make_content_range_header_field(req.ranges[0], res.body.size());
            res.set_header("Content-Range", content_range);
...
inline bool Server::dispatch_request_for_content_reader(Request& req, Response& res,
                                                        ContentReader content_reader,
                                                        const HandlersForContentReader& handlers)
{
    for (const auto& x : handlers)
...
inline socket_t ClientImpl::create_client_socket(Error& error) const
{
    if (!proxy_host_.empty() && proxy_port_ != -1)
    {
        return detail::create_client_socket(
            proxy_host_, std::string(), proxy_port_, address_family_, tcp_nodelay_, socket_options_,
    ...
    return detail::create_client_socket(
        host_, ip, port_, address_family_, tcp_nodelay_, socket_options_, connection_timeout_sec_,
        connection_timeout_usec_, read_timeout_sec_, read_timeout_usec_, write_timeout_sec_,
        ...
        if (!username.empty() && !password.empty())
        {
            ...
            if (detail::parse_www_authenticate(res, auth, is_proxy))
            {
                ...
                new_req.headers.insert(detail::make_digest_authentication_header(
                    req, auth, new_req.authorization_count_, detail::random_string(10), username,
                    password, is_proxy));
...
inline bool ClientImpl::write_content_with_provider(Stream& strm, const Request& req, Error& error)
{
    ...
    {
        return detail::write_content(strm, req.content_provider_, 0, req.content_length_,
...
ClientImpl::send_with_content_provider(Request& req, const char* body, size_t content_length,
                                       ContentProvider content_provider,
                                       ContentProviderWithoutLength content_provider_without_length,
                                       const std::string& content_type, Error& error)
{
...
inline Result ClientImpl::send_with_content_provider(
    const std::string& method, const std::string& path, const Headers& headers, const char* body,
    size_t content_length, ContentProvider content_provider,
    ContentProviderWithoutLength content_provider_without_length, const std::string& content_type)
{
    Request req;
    req.method = method;
    req.headers = headers;
                                        ...
                                        bool close_connection, Error& error)
{
    ...
    if ((res.status != 204) && req.method != "HEAD" && req.method != "CONNECT")
    {
        ...
        int dummy_status;
        if (!detail::read_content(strm, res, (std::numeric_limits<size_t>::max)(), dummy_status,
...
ClientImpl::get_multipart_content_provider(const std::string& boundary,
                                           const MultipartFormDataItems& items,
                                           const MultipartFormDataProviderItems& provider_items)
{
...
inline Result ClientImpl::Get(const std::string& path) { return Get(path, Headers(), Progress()); }
...
inline Result ClientImpl::Post(const std::string& path, const Headers& headers, const char* body,
                               size_t content_length, const std::string& content_type)
{
    return send_with_content_provider("POST", path, headers, body, content_length, nullptr, nullptr,
                                      content_type);
}
...
{
    return send_with_content_provider("POST", path, headers, body.data(), body.size(), nullptr,
...
inline Result ClientImpl::Post(const std::string& path, const Headers& headers,
                               ...
                               const std::string& content_type)
{
    return send_with_content_provider("POST", path, headers, nullptr, content_length,
...
inline Result ClientImpl::Put(const std::string& path, const Headers& headers, const char* body,
                              size_t content_length, const std::string& content_type)
{
    return send_with_content_provider("PUT", path, headers, body, content_length, nullptr, nullptr,
                                      content_type);
}
...
inline Result ClientImpl::Put(const std::string& path, const Headers& headers,
                              const std::string& body, const std::string& content_type)
{
    return send_with_content_provider("PUT", path, headers, body.data(), body.size(), nullptr,
                                      nullptr, content_type);
}
...
inline Result ClientImpl::Put(const std::string& path, const Headers& headers,
                              ...
                              const std::string& content_type)
{
    return send_with_content_provider("PUT", path, headers, nullptr, content_length,
                                      std::move(content_provider), nullptr, content_type);
}
...
inline Result ClientImpl::Patch(const std::string& path, const Headers& headers, const char* body,
                                size_t content_length, const std::string& content_type)
{
    return send_with_content_provider("PATCH", path, headers, body, content_length, nullptr,
                                      nullptr, content_type);
}
...
inline Result ClientImpl::Patch(const std::string& path, const Headers& headers,
                                const std::string& body, const std::string& content_type)
{
    return send_with_content_provider("PATCH", path, headers, body.data(), body.size(), nullptr,
                                      nullptr, content_type);
}
...
inline Result ClientImpl::Patch(const std::string& path, const Headers& headers,
                                ...
                                const std::string& content_type)
{
    return send_with_content_provider("PATCH", path, headers, nullptr, content_length,
                                      std::move(content_provider), nullptr, content_type);
}
...
inline SSL* ssl_new(socket_t sock, SSL_CTX* ctx, std::mutex& ctx_mutex, U SSL_connect_or_accept,
                    V setup)
{
    SSL* ssl = nullptr;
    {
...
template <typename T>
inline bool process_client_socket_ssl(SSL* ssl, socket_t sock, time_t read_timeout_sec,
                                      time_t read_timeout_usec, time_t write_timeout_sec,
                                      time_t write_timeout_usec, T callback)
{
    SSLSocketStream strm(sock, ssl, read_timeout_sec, read_timeout_usec, write_timeout_sec,
...
inline SSLSocketStream::SSLSocketStream(socket_t sock, SSL* ssl, time_t read_timeout_sec,
                                        time_t read_timeout_usec, time_t write_timeout_sec,
...
inline bool SSLServer::process_and_close_socket(socket_t sock)
{
    auto ssl = detail::ssl_new(
        ...
        [&](SSL* ssl2)
        {
            return detail::ssl_connect_or_accept_nonblocking(sock, ssl2, SSL_accept,
    ...
    if (ssl)
    {
        ret = detail::process_server_socket_ssl(
            svr_sock_, ssl, sock, keep_alive_max_count_, keep_alive_timeout_sec_, read_timeout_sec_,
            read_timeout_usec_, write_timeout_sec_, write_timeout_usec_,
                                          ...
                                          Error& error)
{
    ...
    if (!detail::process_client_socket(socket.sock, read_timeout_sec_, read_timeout_usec_,
                                       ...
                                       [&](Stream& strm)
                                       {
                                           ...
                                           return process_request(strm, req2, proxy_res, false,
                                                                  error);
                                       }))
    {
                ...
                if (!detail::process_client_socket(
                        ...
                        [&](Stream& strm)
                        {
                            ...
                            req3.headers.insert(detail::make_digest_authentication_header(
                                ...
                                proxy_digest_auth_username_, proxy_digest_auth_password_, true));
...
inline bool SSLClient::initialize_ssl(Socket& socket, Error& error)
{
    auto ssl = detail::ssl_new(
        ...
        [&](SSL* ssl2)
        {
            ...
            if (!detail::ssl_connect_or_accept_nonblocking(socket.sock, ssl2, SSL_connect,
...
{
    assert(socket.ssl);
    return detail::process_client_socket_ssl(socket.ssl, socket.sock, read_timeout_sec_,
...
inline Result Client::Delete(const std::string& path) { return cli_->Delete(path); }

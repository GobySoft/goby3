...
// ZMQReadThread
//
goby::ZMQReadThread::ZMQReadThread(const protobuf::InterProcessPortalConfig& cfg,
                                   zmq::context_t& context, std::atomic<bool>& alive,
                                   std::shared_ptr<std::condition_variable_any> poller_cv)
    : cfg_(cfg), control_socket_(context, ZMQ_PAIR), subscribe_socket_(context, ZMQ_SUB),
      manager_socket_(context, ZMQ_REQ), alive_(alive), poller_cv_(poller_cv)
{
...
void goby::ZMQManager::run()
{
    ...
    try
    {
        while (true)
        {
            ...
            while (cfg_.transport() == goby::protobuf::InterProcessPortalConfig::TCP &&

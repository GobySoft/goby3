...
///
/// Runs on whichever thread is servicing the transporter callback.
...
///
/// Only plain data, so that the handler stays async-signal-safe.
struct SignalState
{
...
//
// GROUP_KEY is the interface.yml expression for GROUP as a string, which is the only name
// Python has for the group; a group's runtime name need not be its C++ variable name.

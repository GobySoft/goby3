...
namespace julia
{
// the language-neutral types live in goby::middleware::languages; these aliases keep the names
// used by the generated Julia glue code (goby::middleware::julia::Identifier, etc.) working
using goby::middleware::languages::Identifier;
using goby::middleware::languages::PubSubLayer;
using goby::middleware::languages::serialize_uint8;
...
//
// GROUP_KEY is the interface.yml expression for GROUP as a string, which is the only name Julia
// has for the group; a group's runtime name need not be its C++ variable name.
#define GOBY_JULIA_IF_PUBLICATION(SCHEME, LAYER_ENUM, LAYER_FUNCTION, GROUP, GROUP_KEY, TYPE)     \
...
#define GOBY_JULIA_IF_SUBSCRIPTION(SCHEME, LAYER_ENUM, LAYER_FUNCTION, GROUP, GROUP_KEY, TYPE) \
    if (id == goby::middleware::julia::Identifier{                                              \
                  goby::middleware::julia::PubSubLayer::LAYER_ENUM, TYPE::descriptor()->name(), \
                  goby::middleware::MarshallingScheme::SCHEME, GROUP_KEY})                      \
    {                                                                                           \

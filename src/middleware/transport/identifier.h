...
class InterProcessIdentifierManager
{
  public:
    ...
    // returns tuple: group, scheme, type, process, thread
    static std::tuple<std::string, int, std::string, int, std::size_t>
  ...
  protected:
    ...
    std::string _make_identifier(const std::string& type_name, int scheme,
                                 const std::string& group, IdentifierWildcard wildcard)
    {

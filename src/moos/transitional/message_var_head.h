...
class DCCLMessageVarHead : public DCCLMessageVarInt
{
  public:
    DCCLMessageVarHead(const std::string& default_name, int bit_size)
        : DCCLMessageVarInt(pow(2, bit_size) - 1, 0), bit_size_(bit_size),
...
class DCCLMessageVarTime : public DCCLMessageVarHead
{
  public:
    ...
    void set_defaults_specific(DCCLMessageVal& v, unsigned modem_id, unsigned id)
    {
        ...
        v = (!v.empty() && v.get(d))
                ? v
                : DCCLMessageVal(common::ptime2unix_double(common::goby_time()));
    }
    ...
    void pre_encode(DCCLMessageVal& v)
    {
        double d;
        v = (!v.empty() && v.get(d))
                ? DCCLMessageVal(goby::util::as<std::string>(goby::common::unix_double2ptime(d)))
                : v;
    }
    ...
    {
        std::string s;
        v = (!v.empty() && v.get(s)) ? DCCLMessageVal(goby::common::ptime2unix_double(
                                           goby::util::as<boost::posix_time::ptime>(s)))
                                     : v;
    }

    void write_schema_to_dccl2(std::ofstream* proto_file, int sequence_number)
    {
        sequence_number_ = sequence_number;
        *proto_file << "\t"
                    << "optional double " << name() << " = " << sequence_number
...
class DCCLMessageVarSrc : public DCCLMessageVarHead
{
  public:
    ...
    virtual std::string additional_option_extensions() { return "(dccl.field).in_head=true"; }
};

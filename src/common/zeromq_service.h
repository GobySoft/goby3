...
class ZeroMQSocket
{
  public:
    ZeroMQSocket()
        : global_blackout_(boost::posix_time::not_a_date_time), local_blackout_set_(false),
          global_blackout_set_(false)
    {
    }

    ZeroMQSocket(boost::shared_ptr<zmq::socket_t> socket)
        : socket_(socket), global_blackout_(boost::posix_time::not_a_date_time),
          local_blackout_set_(false), global_blackout_set_(false)
    {
    }

    void set_global_blackout(boost::posix_time::time_duration duration);
    void set_blackout(int marshalling_scheme, const std::string& identifier,
                      boost::posix_time::time_duration duration);

    void clear_blackout(int marshalling_scheme, const std::string& identifier)
    {
    ...
    bool check_blackout(int marshalling_scheme, const std::string& identifier);

    void set_socket(boost::shared_ptr<zmq::socket_t> socket) { socket_ = socket; }

    boost::shared_ptr<zmq::socket_t>& socket() { return socket_; }

  private:
    struct BlackoutInfo
    {
        BlackoutInfo(boost::posix_time::time_duration interval = boost::posix_time::not_a_date_time)
            : blackout_interval(interval), last_post_time(boost::posix_time::neg_infin)
        {
        }
...
class ZeroMQService
{
  public:
    ...
    void send(int marshalling_scheme, const std::string& identifier, const std::string& body,
              int socket_id);

    void subscribe(int marshalling_scheme, const std::string& identifier, int socket_id);

    void unsubscribe(int marshalling_scheme, const std::string& identifier, int socket_id);

    template <class C>
    void connect_inbox_slot(void (C::*mem_func)(int, const std::string&, const std::string&, int),
                            C* obj)
    {
        goby::glog.is(goby::common::logger::DEBUG1) &&
            goby::glog << "ZeroMQService: made connection for: " << typeid(obj).name() << std::endl;
        connect_inbox_slot(boost::bind(mem_func, obj, _1, _2, _3, _4));
    }

    void
    connect_inbox_slot(boost::function<void(int marshalling_scheme, const std::string& identifier,
    ...
    template <class C>
    void register_poll_item(const zmq::pollitem_t& item, void (C::*mem_func)(const void*, int, int),
                            C* obj)
    {
    ...
    void
    register_poll_item(const zmq::pollitem_t& item,
    ...
    boost::signals2::signal<void(int marshalling_scheme, const std::string& identifier,
                                 int socket_id)>
        pre_send_hooks;

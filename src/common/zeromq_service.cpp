    ...
    for (int i = 0, n = cfg.socket_size(); i < n; ++i)
    {
        ...
        else if (cfg.socket(i).connect_or_bind() == protobuf::ZeroMQServiceConfig::Socket::BIND)
        {
            ...
            try
            {
                ...
                int rc = zmq_getsockopt((void*)*this_socket, ZMQ_LAST_ENDPOINT, &last_endpoint,
                                        &last_endpoint_size);
                    ...
                    cfg.socket(i).ethernet_port() == 0)
                {
                    ...
                    cfg.mutable_socket(i)->set_ethernet_port(
                        std::stoi(last_ep.substr(last_ep.find_last_of(":") + 1)));
                }

                glog.is(DEBUG1) &&
                    glog << group(glog_out_group()) << "bound to endpoint - " << last_endpoint
...
void goby::common::ZeroMQService::subscribe(int marshalling_scheme, const std::string& identifier,
                                            int socket_id)
{
...
void goby::common::ZeroMQService::unsubscribe(int marshalling_scheme, const std::string& identifier,
                                              int socket_id)
{
    std::string zmq_filter = zeromq_packet_make_header(marshalling_scheme, identifier);
    int NULL_TERMINATOR_SIZE = 1;
...
void goby::common::ZeroMQService::send(int marshalling_scheme, const std::string& identifier,
...
void goby::common::ZeroMQSocket::set_blackout(int marshalling_scheme, const std::string& identifier,

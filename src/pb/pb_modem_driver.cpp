...
goby::pb::PBDriver::PBDriver(goby::common::ZeroMQService* zeromq_service)
    : StaticProtobufNode(zeromq_service), zeromq_service_(zeromq_service),
      last_send_time_(goby_time<std::uint64_t>()), request_socket_id_(0),
      query_interval_seconds_(1), reset_interval_seconds_(120), waiting_for_reply_(false),
      next_frame_(0)
{
...
void goby::pb::PBDriver::startup(const acomms::protobuf::DriverConfig& cfg)
{
    ...
    service_cfg_.add_socket()->CopyFrom(driver_cfg_.GetExtension(PBDriverConfig::request_socket));
    zeromq_service_->set_cfg(service_cfg_);

    request_socket_id_ = driver_cfg_.GetExtension(PBDriverConfig::request_socket).socket_id();

    query_interval_seconds_ = driver_cfg_.GetExtension(PBDriverConfig::query_interval_seconds);

    reset_interval_seconds_ = driver_cfg_.GetExtension(PBDriverConfig::reset_interval_seconds);
}

void goby::pb::PBDriver::shutdown() {}

void goby::pb::PBDriver::handle_initiate_transmission(
    const acomms::protobuf::ModemTransmission& orig_msg)
{
            ...
            if (driver_cfg_.modem_id() == msg.src())
            {
                ...
                if (msg.rate() < driver_cfg_.ExtensionSize(PBDriverConfig::rate_to_bytes))
                    msg.set_max_frame_bytes(
                        driver_cfg_.GetExtension(PBDriverConfig::rate_to_bytes, msg.rate()));
                else
                    msg.set_max_frame_bytes(
                        driver_cfg_.GetExtension(PBDriverConfig::max_frame_size));

                if (msg.rate() < driver_cfg_.ExtensionSize(PBDriverConfig::rate_to_frames))
                    msg.set_max_num_frames(
                        driver_cfg_.GetExtension(PBDriverConfig::rate_to_frames, msg.rate()));
...
{
    glog.is(DEBUG1) && glog << group(glog_in_group()) << "Received response in "
                            << (goby_time<std::uint64_t>() - last_send_time_) / 1.0e6 << " seconds."
                            << std::endl;
    ...
    for (int i = 0, n = response.inbox_size(); i < n; ++i)
    {
        ...
        else
        {
            ...
            if (msg.dest() == driver_cfg_.modem_id() &&
                msg.type() == acomms::protobuf::ModemTransmission::DATA && msg.ack_requested())
            {
